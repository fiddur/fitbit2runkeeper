...
const ConvertFb2Hg      = require('./convertFb2Hg')

const Fitbit = require('./fitbit')

// const rp = params => {
//   console.log('--->>>  rp request: ', params)
//   return realRp(params)
//     .then(response => {
...
module.exports = (config, users) => {
  const onNewTokens = users.setFitbitTokens
  // const onNewTokens = (id, tokens) => users.getById(id)
  //       .then(user => {
  //         console.log('onNewTokens before', { id, fitbit: user.accounts.fitbit, tokens })
  //         return users.setFitbitTokens(id, tokens)
  //       })
  //       .then(() => users.getById(id))
  //       .then(user => {
  //         console.log('onNewTokens after', { id, fitbit: user.accounts.fitbit, tokens })
  //         return user
  //       })

  const cache = {}
  const fitbit = Fitbit({ config: config.fitbit, onNewTokens, rp })
  const convertFb2Hg = ConvertFb2Hg(cache)
  ...
  const onAuthenticated = aasync((req, accessToken, refreshToken, profile, done) => {
    // console.log('onAuthenticated', { accessToken, refreshToken })
    const user = req.user
  ...
  app.get(
    '/home', aasync((req, res) => {
      ...
      if (req.user.exported) {
        activities.forEach(
          activity => { activity.exported = req.user.exported[activity.logId] }
        )
      }
  ...
  app.post('/fitbit/webhook', (req, res) => {
    // Send an early OK to not timeout.
    res.sendStatus(204)
      ...
      req.body
        ...
        .map(
          updated => users
            ...
            .then(user => syncUserAtDate({ user, date: moment(updated.date) }))
        )
    ).then(all => {
      console.log('All', all)
      // res.sendStatus(204)
